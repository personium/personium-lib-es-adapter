...
import io.personium.common.es.impl.EsTranslogHandler;
...
import io.personium.common.es.response.implnew.PersoniumGetResponseImpl;
import io.personium.common.es.response.implnew.PersoniumIndexResponseImpl;
import io.personium.common.es.response.implnew.PersoniumMappingMetaDataImpl;
...
/**
 * Type 操作用 Class. ES6 以降、タイプレスAPIとなるためType別にインデックスを作成する。
 */
public class EsTypeImpl
        // extends EsTranslogHandler
        implements EsType {
    ...
    private ElasticsearchAsyncClient aClient;

    /**
     * コンストラクタ.
     * @param index インデックス名
     * @param typeName type名
     ...
     */
    public EsTypeImpl(String index,
            String typeName,
            String routingId,
            int times,
            int interval,
            RestClientTransport transport) {
        // super(times, interval, , index);
        ...
        this.aClient = new ElasticsearchAsyncClient(transport);
    }
    ...
    protected String composeIndexName() {
        return PersoniumEsIndexImpl.composePersoniumIndexString(this.indexName, this.typeName);
    }
     ...
     * getDocument by Id.
     * @param id document id
     * @param realtime flag switching realtime mode
     */
    @Override
    public PersoniumGetResponse get(final String id, final boolean realtime) {
        ...
        try {
            // update document by re-index.
            String index = composeIndexName();
            GetResponse<ObjectNode> gr = esClient
                    .get(req -> req.index(index).id(id).routing(this.routingId).realtime(realtime), ObjectNode.class);
            return PersoniumGetResponseImpl.getInstance(gr);
        } catch (IOException e) {
    ...
    public PersoniumIndexResponse create(final String id, @SuppressWarnings("rawtypes") final Map data) {
        ...
        try {
            String index = composeIndexName();
            CreateResponse cr = esClient.create(r -> r.index(index).id(id).routing(this.routingId).document(data));
            return PersoniumIndexResponseImpl.getInstance(cr);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
     ...
     */
    @Override
    public PersoniumIndexResponse update(final String id,
            @SuppressWarnings("rawtypes") final Map data,
            final long version) {
        ...
        try {
            // update document by re-index.
            String index = composeIndexName();
            CreateResponse cr = esClient
                    .create(r -> r.index(index).id(id).routing(this.routingId).document(data).version(version));
            return PersoniumIndexResponseImpl.getInstance(cr);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    ...
    public PersoniumMappingMetaData getMapping() {
        try {
            String index = composeIndexName();
            GetMappingResponse gmr = esClient.indices().getMapping(mr -> mr.index(index));
            // return PersoniumMappingMetaDataImpl.getInstance(esClient.getMapping(this.indexName, this.name));
            IndexMappingRecord imr = gmr.get(index);
            imr.mappings().properties();
            return PersoniumMappingMetaDataImpl.getInstance(gmr.get(index));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
     ...
     */
    class GetRetryableRequest extends AbstractRetryableEsRequest<org.elasticsearch.action.get.GetResponse> {
        String id;
        boolean realTime;

        GetRetryableRequest(int retryCount, long retryInterval, String argId, boolean argRealTime) {
        ...
        @Override
        org.elasticsearch.action.get.GetResponse doProcess() {
            org.elasticsearch.action.get.GetResponse response = asyncGet(id, realTime).actionGet();
        ...
        @Override
        org.elasticsearch.action.get.GetResponse onParticularError(ElasticsearchException e) {
    ...
    class CreateRetryableRequest extends AbstractRetryableEsRequest<IndexResponse> {
        ...
        CreateRetryableRequest(int retryCount,
                long retryInterval,
                String argId,
                @SuppressWarnings("rawtypes") Map argData) {
        ...
        @Override
        boolean isParticularError(ElasticsearchException e) {
            return e instanceof VersionConflictEngineException || e instanceof IndexNotFoundException
                    || e.getCause() instanceof IndexNotFoundException || e instanceof MapperParsingException;
        }
    ...
    class UpdateRetryableRequest extends AbstractRetryableEsRequest<IndexResponse> {
        ...
        UpdateRetryableRequest(int retryCount, long retryInterval, String argId, Map argData, long argVersion) {
        ...
        boolean isParticularError(ElasticsearchException e) {
            return e instanceof IndexNotFoundException || e.getCause() instanceof IndexNotFoundException
                    || e instanceof VersionConflictEngineException || e instanceof MapperParsingException;
        }
     ...
     * @return 非同期応答
     */
    public ActionFuture<org.elasticsearch.action.get.GetResponse> asyncGet(final String id, final boolean realtime) {
        // return esClient.asyncGet(this.indexName, this.typeName, id, this.routingId, realtime);
        return null;
    }
    ...
    public ActionFuture<SearchResponse> asyncSearch(final SearchSourceBuilder builder) {
        // return esClient.asyncSearch(this.indexName, this.typeName, this.routingId, builder);
        return null;
    }
    ...
    public ActionFuture<SearchResponse> asyncSearch(final Map<String, Object> query) {
        // return esClient.asyncSearch(this.indexName, this.typeName, this.routingId, query);
        return null;
    }
    ...
    public ActionFuture<MultiSearchResponse> asyncMultiSearch(final List<Map<String, Object>> queryList) {
        // return esClient.asyncMultiSearch(this.indexName, this.typeName, this.routingId, queryList);
        return null;
    }
    ...
    public ActionFuture<IndexResponse> asyncIndex(final String id,
        ...
        // return esClient.asyncIndex(this.indexName, this.typeName, id, this.routingId, data, opType, version);
        return null;
    }
    ...
    public ActionFuture<DeleteResponse> asyncDelete(final String id, final long version) {
        // return esClient.asyncDelete(this.indexName, this.typeName, id, this.routingId, version);
        return null;
    }
    ...
    public ActionFuture<AcknowledgedResponse> asyncPutMapping(Map<String, Object> mappings) {
        // return esClient.putMapping(this.indexName, this.typeName, mappings);
        return null;
    }
